...
void main() {
    ...
    vec3 vertexPos = (ubo.matrix_view * ubo.matrix_model * vec4(inPosition, 1.0)).xyz;
    vec3 normal= normalize(mat3(ubo.matrix_normals) * inNormal);

    // NOTE i might need a minus here
    ...
    if (dot(normal, viewDirection) >= 0) {
        ...
        vec3 directionalLighting = computeDirectionalLighting(vertexPos, actualNormal, viewDirection);

        // Point light (view space)
        vec3 pointLighting = computePointLighting(vertexPos, actualNormal, viewDirection);
    ...
    if (ubo.drawModes.x == 1) {
        ...
        vec3 scaledNormal = 0.5 * normal + 0.5;
        fragColor = vec4(
